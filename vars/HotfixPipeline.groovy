                    ...
                    stage("Started Deployment to PROD") {
                        steps {
                            ...
                            script {
                                if (config.build) {
                                    stage ('npm build') {
                                        sh '[ -f node_modules/prod.config.js ] && mv node_modules/prod.config.js src/config/config.js'
                                        sh 'npm run build'
                                    }
                                }
                                if(config.copyConfig) {
                                    stage ('copy config') {
                                        sh '[ -f node_modules/prod.config.ts ] && mv node_modules/prod.config.ts src/config/config.ts'
                                    }
                                }
                            }
